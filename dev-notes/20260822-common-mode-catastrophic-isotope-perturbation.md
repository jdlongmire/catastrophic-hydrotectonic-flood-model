...
- **New, from the super-summary:** find and read in primary the literature on secondary/metamorphic U-Pb concordance — what magnitude of apparent age has actually been produced by a documented secondary process, and by what mechanism (new zircon growth incorporating an inherited core reads differently from whole-grain Pb-loss-then-regrowth). The Jack Hills zircon record specifically should be checked rather than assumed as the target case, since it is largely reported as discordant, not concordant, in the primary literature — the super-summary's "concordant ... beyond four billion years" claim needs its own sourced case before it can be steelmanned or tested.
