...
![Impact-triggered failure sequence](../assets/images/initial-meteor-event.png)
...
**Figure 3.** The proposed breakup sequence (pre-impact supercontinent through post-catastrophic relaxation) and continental positions through time. Revised twice on 2026-08-19; supersedes the earlier `continental-shift` pair and a first revision, both archived. The current figure names the supercontinent Pangaea, renders the trigger as a distributed impact swarm rather than a single impact, states pore pressure as approaching the lithostatic limit from below (consistent with the pore-pressure ratio derived in Section 3.4), and states plate velocity as 9 to 40 cm/s — this programme's own kinematics figure, resolving a conflict that had previously required cropping the source infographic for paper use. Displacement distances (2,000 to 6,000 km average, 8,000 to 12,000+ km maximum) are restored in the banner. Full audit in `assets/images/README.md`. Illustrative of sequence and geometry only; not a validated plate-motion reconstruction (Section 3.4), and the subduction it depicts is labelled proposed because it remains subject to the unresolved rheology gate.
...
### 3.4 Velocity and detachment depth — reconciled 2026-08-18

The lateral-velocity claim carried a real internal inconsistency, disclosed rather than smoothed over, and now resolved (`WP-HTF-0003`). The source document's summary figure ("tens to hundreds of meters per hour," approximately 0.03 to 0.1 m/s) did not match its own worked force-balance calculation, which for a representative 800 × 1000 × 35 km block on a 0.1° slope with friction collapsed to $\mu = 0.01$ yielded an acceleration of approximately 0.03 m/s² and, over a 15-minute interval, a velocity of approximately 30 m/s: roughly 1,000 times faster than the summary claim, a discrepancy that traces in part to a unit-conversion error in the source (30 m/s was there reported as "108 m/hr," when the correct conversion is 108,000 m/hr). Detachment depth was likewise stated inconsistently in the source (50 km in the narrative text, 15 km in the worked calculation).

Redone against this programme's own parameters rather than the source's block dimensions or μ/slope combination (`02-theory/kinematics/code/velocity_detachment_reconciliation.py`), three findings survive. **The disputed 30 m/s figure is rejected**, now on three independent grounds: the unit-conversion error above; refutation on energy conservation (Section 3.2's own budget cannot support it, `02-theory/rheology/README.md` F2); and a driving-force inconsistency — $\tan(0.1°) \approx 0.00175$ is less than the source's own stated $\mu = 0.01$, so the source's downslope-gravity model cannot even initiate sliding on its own numbers. **This programme's own existing figure, 9 to 40 cm/s, is the surviving velocity**, unchanged, and passes the energy-ceiling check (Section 4, F2) with a 15-fold margin. The driving force is read as isostatic / gravitational-potential-energy release — consistent with this paper's own Section 3.2 partition (94.2% residual PE) — rather than downslope gravity on a shallow global slope. **Detachment depth is set at 15 km**, above this programme's own crustal-thickness figure and consistent with the conventional brittle-ductile transition depth, in preference to 50 km, which sits at or below the Moho.

Solving for the pore-pressure ratio the Terzaghi-collapse mechanism requires to dissipate exactly the published friction allocation (5% of the Section 3.2 budget) over the programme's own tabled continental travel distances gives $\lambda = 0.999995$ to $0.9999996$ — five to six nines, past typically documented overpressure ratios in the fault-mechanics literature. This programme reads that as a demanding but genuine feature of a designed, singular historical event rather than a defect in the mechanism or the imported energy partition (`04-construct/methods.md` rule 21).
...
![Water-rich porous crust cross-section](../assets/images/initial-supercontinental-crust-model.png)
