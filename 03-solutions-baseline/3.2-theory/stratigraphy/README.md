...
JD's same-day super-summary sharpened this to its hardest case — a single common event producing concordant apparent ages beyond 4 Ga alongside substantial discordance elsewhere — then, prompted by the framing tension that raised, generalized it: "substantial populations of concordant apparent ages," decay-constant invariance stated inline, deliberately agnostic between two branches with different burdens (untouched $R_0$ inheritance, vs. actively manufactured concordance). Concordance across both U decay chains is normally read as the *reliability* signal, the standing argument against secondary-disturbance accounts, which is why the branch choice matters before either can be checked.

**Not yet checked:** which branch (or mixture) is actually claimed; whether this resolves the two problems above or merely restates them as free parameters in $f$; whether the discriminator has a defined, testable dataset (condition 3); whether a sourced case for secondary concordance at deep-time magnitude actually exists; and the mainstream discordia/Pb-loss/resetting-age literature has not been steelmanned against it. See the dev-note's Open section.
