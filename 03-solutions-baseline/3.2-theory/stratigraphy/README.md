...
JD's same-day super-summary sharpens this to its hardest case: a single common event producing **concordant apparent ages beyond 4 Ga** (the oldest end of the U-Pb record, e.g. the Jack Hills detrital zircons) alongside substantial discordance elsewhere. This is a materially harder claim than plain discordance — concordance across both U decay chains is normally read as the *reliability* signal, the standing argument against exactly this kind of secondary-disturbance account, and zircon's high retentivity is specifically why it is used for old ages.

**Not yet checked:** whether this actually resolves the two problems above or merely restates them as free parameters in $f$; whether the discriminator has a defined, testable dataset (condition 3); whether a sourced case for secondary concordance at >4 Ga magnitude actually exists (the Jack Hills record is largely reported as discordant, not concordant, in the primary literature); and the mainstream discordia/Pb-loss/resetting-age literature has not been steelmanned against it. See the dev-note's Open section.
