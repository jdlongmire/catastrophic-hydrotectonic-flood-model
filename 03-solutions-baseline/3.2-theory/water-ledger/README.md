...
**How the free parameters here should be read** — [`research-practices.md`](../../../04-construct/methods.md) **rule 22**, added 2026-08-19 on JD's pushback that naturalistic models routinely adjust knobs. Swarm timing, coupling fraction, loft height, and salinity fraction are free parameters, and that is **not by itself a defect** — mantle viscosity profiles, the quantity this programme's own rheology gate is benchmarked against, are inverted from data rather than derived. The standard is over-determination: a free parameter becomes a problem only when no evaluated constraint bears on it. This programme's actual position is many free parameters against **zero evaluated constraints**, which is the same kind of situation as any young programme's, and the remedy is more evaluated constraints rather than fewer parameters. Rule 21 does **not** cover these — it governs derived values landing at demanding points, not free parameters.
